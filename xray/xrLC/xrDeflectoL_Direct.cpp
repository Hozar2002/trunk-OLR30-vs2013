...
void CDeflector::L_Direct_Edge (CDB::COLLIDER* DB, base_lighting* LightsSelected, Fvector2& p1, Fvector2& p2, Fvector& v1, Fvector& v2, Fvector& N, float texel_size, Face* skip)
{
	...
	for (int I=0; I<=steps; I++)
	{
		...
		LightPoint		(DB, RCAST_Model, C, P, N, *LightsSelected, (b_norgb?LP_dont_rgb:0)|(b_nosun?LP_dont_sun:0)|LP_DEFAULT, skip); //.  ÂÎÒ ÒÓÒ ÂÛÇÎÂ  ! íå ñîëíöå !
				...
				for (u32 J=0; J<Jcount; J++) 
				{
					...
					for (UVtri** it=&*space.begin(); it!=&*space.end(); it++)
					{
						if ((*it)->isInside(P,B)) {
							...
							//. íå íóæíî èñïîëüçîâàòü	if (F->Shader().flags.bLIGHT_Sharp)	{ wN.set(F->N); }
							//	else								
							{ 
							...
							try {
								LightPoint(DB, RCAST_Model, C, wP, wN, *LightsSelected, (b_norgb?LP_dont_rgb:0)|(b_nosun?LP_dont_sun:0)|LP_UseFaceDisable, F); //.  ÂÎÒ ÒÓÒ ÂÛÇÎÂ !!! ÂÐÎÄÅ ÑÎËÍÖÅ !!!
								//int i = 0;
								//for (i = 1; i <= 10; i++){
