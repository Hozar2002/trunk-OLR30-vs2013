...
{
	auto pInvOwner = smart_cast<CInventoryOwner*>(this);
	THROW(pInvOwner);
	
	auto pOthersGO = smart_cast<CGameObject*>(phrase_dialog->OurPartner(this));
	THROW(pOthersGO);
	
	auto pOthersIO = smart_cast<CInventoryOwner*>(pOthersGO);
	THROW(pOthersIO);
	...
	if (!phrase_dialog->IsFinished()) {
		...
		if (pGameSP->TalkMenu->IsShown()) {
			pGameSP->TalkMenu->AddAnswer(
				phrase_dialog->GetPhraseText(phrase_id),
				pInvOwner->Name()
			);
		}
		else {
			luabind::functor<void> script_add_answer;
			R_ASSERT(ai().script_engine().functor("olr_pda._g_add_answer",script_add_answer));
			script_add_answer(
				phrase_dialog->GetPhraseText(phrase_id),
				pInvOwner->Name()
			);
