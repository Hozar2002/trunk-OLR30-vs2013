...
CActor::CActor() : CEntityAlive(), m_bControllerImpact{}
{
...
void	CActor::Hit(SHit* pHDS)
{
	...
	// hit physics actor
	if (HDS.hit_type == ALife::eHitTypeWound || HDS.hit_type == ALife::eHitTypeExplosion || HDS.hit_type == ALife::eHitTypePhysicStrike){
		Actor()->character_physics_support()->movement()->ApplyImpulse(HDS.dir, Actor()->GetMass() * HDS.impulse * skel_fatal_impulse_factor);
	};


	//if (HDS.hit_type == ALife::eHitTypeWound || HDS.hit_type == ALife::eHitTypeExplosion || HDS.hit_type == ALife::eHitTypePhysicStrike)
	//{
	//	Actor()->character_physics_support()->movement()->ApplyImpulse(HDS.dir, HDS.impulse);
	//	m_saved_dir.set(HDS.dir);
	//	m_saved_position.set(HDS.bone_space_position);
	//	m_saved_impulse = HDS.impulse*skel_fatal_impulse_factor;
	//	m_saved_element = HDS
	//}
...
{
	// hit marker
	if ( /*(hit_type==ALife::eHitTypeFireWound||hit_type==ALife::eHitTypeWound_2) &&*/ g_Alive() && Local() && /*(this!=who) && */(Level().CurrentEntity() == this))	// комент типа хита
	{
		Fvector l_d;
		l_d.set(dir);
		l_d.normalize();
		Cameras().AddPPEffector(xr_new<CShootingHitEffectorPP>(m_pShootingEffector->ppi, m_pShootingEffector->time, m_pShootingEffector->time_attack, m_pShootingEffector->time_release));
		Cameras().AddCamEffector(xr_new<CShootingHitEffector>(XFORM().i.dotproduct(l_d), XFORM().j.dotproduct(l_d), P));
		HUD().Hit(0, P, dir);

		{
			...
			if (P > 0.05f){
				...
				{
					//Msg("0 front");
					SDrawStaticStruct* s = HUD().GetUI()->UIGame()->AddCustomStatic("pcfm_hitmark_f", false);
					s->m_endTime = Device.fTimeGlobal + 0.3f; // 3sec
				}
				else if (id == 1 || id == 6 || id == 7) {
					//Msg("1 right");
					SDrawStaticStruct* s = HUD().GetUI()->UIGame()->AddCustomStatic("pcfm_hitmark_r", false);
					s->m_endTime = Device.fTimeGlobal + 0.3f; // 3sec
				}
				else if (id == 2) {
					//Msg("2 back");
					SDrawStaticStruct* s = HUD().GetUI()->UIGame()->AddCustomStatic("pcfm_hitmark_b", false);
					s->m_endTime = Device.fTimeGlobal + 0.3f; // 3sec
				}
				else if (id == 3 || id == 4 || id == 5) {
					//Msg("3 left");
					SDrawStaticStruct* s = HUD().GetUI()->UIGame()->AddCustomStatic("pcfm_hitmark_l", false);
					s->m_endTime = Device.fTimeGlobal + 0.3f; // 3sec	
				}
				else{
					//Msg("unknown - front");
					//SDrawStaticStruct* s = HUD().GetUI()->UIGame()->AddCustomStatic("pcfm_hitmark_f", false);
					//s->m_endTime = Device.fTimeGlobal+0.4f; // 3sec
				}
			}
			...
			if (!ce)
			{
				...
				AddEffector(this, effFireHit, sect_name, P*0.25f); // ----- hi_flyer
			}
		}
	}
...
void CActor::Die(CObject* who)
{
	...
	if (OnServer())
	{
		...
		for (u32 slot_idx = 0; I != E; ++I, ++slot_idx)
		{
			if (slot_idx == inventory().GetActiveSlot())
			{
				...
				{
					if (IsGameTypeSingle())
						(*I).m_pIItem->SetDropManual(FALSE);
					else
					{
		...
		if (IsGameTypeSingle())
		{
			...
			for (TIItemContainer::iterator l_it = l_rlist.begin(); l_rlist.end() != l_it; ++l_it)
			{
				...
				if ((*l_it)->object().CLS_ID == CLSID_OBJECT_ARMOR)
				{
					(*l_it)->SetDropManual(FALSE);
					continue;
				};
			};
		};
	...
	cam_Set(Random.randI(2) ? eacFreeLook : eacLookAt);
	mstate_wishful &= ~mcAnyMove;
	mstate_real &= ~mcAnyMove;

	if (!g_dedicated_server)
	{
		::Sound->play_at_pos(sndDie[Random.randI(SND_DIE_COUNT)], this, Device.vCameraPosition);
...
void CActor::UpdateCL()
{
	...
	//m_bLanded = character_physics_support()->movement()->gcontact_Power;
...
void CActor::shedule_Update(u32 DT)
{
	...
	// in actor update
	if ((mstate_real&mcSprint) && Level().CurrentControlEntity() == this && (inventory().GetActiveSlot() == 1 || inventory().GetActiveSlot() == 2))
	{
		mstate_wishful &= ~mcSprint;
	}
	...
	if (!input_external_handler_installed() && RQ.O &&  RQ.range<inventory().GetTakeDist()) {
		...
		// hi_flyer 21/03/17
		//Msg("actor looking at: %s", game_object->Name());
		//CGameObject	*object = smart_cast<CGameObject*>(game_object);
		//VERIFY		(object);
		//callback(GameObject::eOnLooking)((smart_cast<CGameObject*>(object))->lua_game_object());
		callback(GameObject::eOnLooking)((smart_cast<CGameObject*>(game_object))->lua_game_object());
...
void CActor::UpdateArtefactsOnBelt()
{
...
#endif
	{
		...
		if (artefact && artefact->GetCondition()>0.01f)
		{
			artefact->ChangeCondition(-artefact->m_fConditioRestore);
	...
	for (TIItemContainer::iterator it = inventory().m_all.begin(); inventory().m_all.end() != it; ++it)
	{
		...
		//Msg("RAD HIT 0: %f", obj->m_fRadiationRestoreSpeed*f_update_time);
		if (obj->m_fRadiationRestoreSpeed > 0.0f){
			conditions().ChangeRadiation(obj->m_fRadiationRestoreSpeed*f_update_time);
			//Msg("RAD HIT: %f", obj->m_fRadiationRestoreSpeed*f_update_time);
		}
	}
	//#endif
