...
CControllerPsyHit::CControllerPsyHit() : m_effector_cam{}, m_effector_pp{}, m_blocked{}, m_bActorOwned{}
{
	m_current_index = 0;
	m_min_tube_dist = 0.f;
}
...
void CControllerPsyHit::reinit()
{
	...
	m_stage[0] = skel->ID_Cycle_Safe("stand_sit_down_attack_0"); VERIFY(m_stage[0]);  //psy_attack_0
	m_stage[1] = skel->ID_Cycle_Safe("z_sit_down_psy_attak_to_stalker_0"); VERIFY(m_stage[1]);  //psy_attack_1
	m_stage[2] = skel->ID_Cycle_Safe("control_attack_0"); VERIFY(m_stage[2]);  //psy_attack_2
	m_stage[3] = skel->ID_Cycle_Safe("sit_stand_up_0"); VERIFY(m_stage[3]); //psy_attack_3
	m_current_index = 0;
...
void CControllerPsyHit::death_glide_start()
{
	...
	SControlDirectionData			*ctrl_dir = (SControlDirectionData*)m_man->data(this, ControlCom::eControlDir);
	VERIFY(ctrl_dir);
	ctrl_dir->heading.target_speed = 30.f;
	ctrl_dir->heading.target_angle = m_man->direction().angle_to_target(Actor()->Position());

	//////////////////////////////////////////////////////////////////////////
	m_bActorOwned = true;
	Actor()->m_bControllerImpact = true;
}
...
void CControllerPsyHit::death_glide_end()
{
	...
	if (m_bActorOwned)
	{
		Actor()->m_bControllerImpact = false;
	}
}
