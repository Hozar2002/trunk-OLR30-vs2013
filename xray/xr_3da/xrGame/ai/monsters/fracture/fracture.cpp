...
void CFracture::Load(LPCSTR section)
{
	...
	::Sound->create(threaten_start_sound,pSettings->r_string(section,"sound_threaten_start"),	st_Effect,SOUND_TYPE_WORLD);
	...
	SVelocityParam &velocity_drag		= move().get_velocity(MonsterMovement::eVelocityParameterDrag);  //-- uncoment


	anim().AddAnim(eAnimLieIdle,		"sit_idle_",			-1, &velocity_none,		PS_LIE	);
	anim().AddAnim(eAnimSleep,			"sit_idle_",			-1, &velocity_none,		PS_LIE	);
	anim().AddAnim(eAnimLieStandUp,		"sit_stand_up_",		-1, &velocity_none,		PS_LIE	);
	anim().AddAnim(eAnimLieToSleep,		"stand_sit_down_",		-1, &velocity_none,		PS_LIE	);
	...
	anim().AddAnim(eAnimThreaten,		"stand_threaten_",		-1, &velocity_none,		PS_STAND,	"stand_die_0", "stand_fx_b", "stand_fx_f", "stand_fx_l");  // NEW (пугание)
...
bool CFracture::check_start_conditions(ControlCom::EControlType type)
{
	...
	//Msg("CFracture::check_start_conditions true");
	return true;
}


void CFracture::on_activate_control(ControlCom::EControlType type)
{
	if (type == ControlCom::eControlThreaten) {
		threaten_start_sound.play_at_pos(this,Position());
		//Msg("izlom activate control");
	}
}

void CFracture::reinit()
{
	inherited::reinit	();
...
//void CFracture::UpdateCL()
//{
	//if (time_upd_timer_fracture > m_dwCurrentTime) return;
	//time_upd_timer_fracture			= m_dwCurrentTime + Random.randI(4000,6000);
    //com_man().seq_run(anim().get_motion_id(eAnimThreaten));
	//Msg("timer izlom");
//}
...
void CFracture::CheckSpecParams(u32 spec_params)
{
	...
	//if (time_upd_timer_fracture < m_dwCurrentTime) {
	//time_upd_timer_fracture			= m_dwCurrentTime + Random.randI(4000,6000);
	//com_man().seq_run(anim().get_motion_id(eAnimThreaten));
	//Msg("timer izlom");
	//}
