...
void CALifeStoryRegistry::add				(ALife::_STORY_ID id, CSE_ALifeDynamicObject *object, bool no_assert)
{
	...
	if (I != m_objects.end()) {
		Msg("!DUBLICATE! Story item ID [%u], Object [%s] at level [%s]",id, object->name_replace(), *ai().game_graph().header().level(ai().game_graph().vertex(object->m_tGraphID)->level_id()).name());
