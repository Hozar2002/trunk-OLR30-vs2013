...
void CALifeSwitchManager::add_online(CSE_ALifeDynamicObject *object, bool update_registries)
{
	...
	if ( object->used_ai_locations() && !ai().level_graph().valid_vertex_id( object->m_tNodeID ) ) {
		Msg( " !!FIX MOB VERTEX!! Trying to correct invalid vertex %u for object %s", object->m_tNodeID, object->name_replace() );
		object->m_tNodeID = ai().level_graph().vertex( object->m_tNodeID, object->o_Position );
		Msg( " !!FIX MOB VERTEX!! new vertex: %u", object->m_tNodeID );
        }
