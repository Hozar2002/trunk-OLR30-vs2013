...
void CEatableItemObject::UseBy(CEntityAlive* entity_alive) {
	...
	entity_alive->conditions().ChangeWater(m_fWaterInfluence);
	entity_alive->conditions().ChangeSleep(m_fSleepInfluence);
...
void CEatableItemObject::GetBriefInfo(xr_string& str_name, xr_string& icon_sect_name, xr_string& str_count, xr_string& ammo_sect_name) {
	str_name = NameShort();
	/*string16 stmp = "";
...
void CEatableItemObject::UpdateXForm() {
	if (Device.dwFrame != dwXF_Frame) {  // ОРИГИНАЛЬНЫЙ КОД
		...
		//Msg("e is [%s]", E->cName());  // 1 раз апдейт при нетспавне вроде, и основной постоянный когда худ активен
	...
	try {
		...
		D.sub(mL.c,mR.c); // вылет
		D.normalize_safe();
		
		R.crossproduct(mR.j,D);  // вылет
		R.normalize_safe();
		
		N.crossproduct(D,R);
		N.normalize_safe();
		
		mRes.set(R,N,D,mR.c);  //вылет + глючит если закоментить
        ...
        Msg                    ("!!!Error in Calculate hud eatable");
    }
