...
void CScriptGameObject::UpdateAvailableDialogs(CScriptGameObject* s_partner) {
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	auto partner = smart_cast<CPhraseDialogManager*>(&s_partner->object());
	if (first && partner) {
		first->UpdateAvailableDialogs(partner);
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call UpdateAvailableDialogs method for non-first && partner object");
	}
}

void CScriptGameObject::SayPhrase(CPhraseDialog* phrase_dialog, LPCSTR phrase_id) {
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		DIALOG_SHARED_PTR phrase_dialog2(phrase_dialog);
		first->SayPhrase(phrase_dialog2, shared_str(phrase_id));
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call SayPhrase method for non-first object");
	}
}

void CScriptGameObject::AvailableDialogsForeach(luabind::functor<void> func) {
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		auto b = first->AvailableDialogs().begin();
		auto e = first->AvailableDialogs().end();
		for ( ; b!=e; ++b) {
			func((*b).get());
		}
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call AvailableDialogsForeach method for non-first object");
	}
}

int CScriptGameObject::GetAvailableDialogsSize() const {
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		return first->GetAvailableDialogsSize();
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call GetAvailableDialogsSize method for non-first object");
		return 0;
	}
}

const CPhraseDialog* CScriptGameObject::GetAvailableDialogsFront() const {
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		return first->AvailableDialogs().front().get();
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call GetAvailableDialogsFront method for non-first object");
		return NULL;
	}
}

const CPhraseDialog* CScriptGameObject::GetDialogByID(LPCSTR dialog_id) const {
	const shared_str sid = shared_str(dialog_id);
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		return first->GetDialogByID(sid).get();
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call GetDialogByID method for non-first object");
		return NULL;
	}
}

bool CScriptGameObject::HaveAvailableDialog(LPCSTR dialog_id) const {
	const shared_str sid = shared_str(dialog_id);
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		return first->HaveAvailableDialog(sid);
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call HaveAvailableDialog method for non-first object");
		return false;
	}
}

void CScriptGameObject::InitDialog(CScriptGameObject* dialog_partner, CPhraseDialog* phrase_dialog) {
	if (dialog_partner==NULL) return;
	auto partner = smart_cast<CPhraseDialogManager*>(&dialog_partner->object());
	auto first = smart_cast<CPhraseDialogManager*>(&object());
	if (first) {
		DIALOG_SHARED_PTR phrase_dialog2(phrase_dialog);
		first->InitDialog(partner, phrase_dialog2);
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call InitDialog method for non-first object");
	}
}

void CScriptGameObject::AddAnswerNews(LPCSTR SpeakerName, LPCSTR str, LPCSTR icon_name, Frect icon_rect) {
	auto actor = smart_cast<CActor*>(&object());
	if (actor) {
		actor->AddAnswerNews(SpeakerName, str, icon_name, icon_rect);
	}
	else {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"ScriptGameObject : attempt to call AddAnswerNews method for non-actor object");
	}
}
...
void CScriptGameObject::ActorPlayHudAnimation(LPCSTR M, bool bMixIn) {
	CWeapon* pWeaponItem = smart_cast<CWeapon*>(&object());
	if (pWeaponItem) {
		CWeaponHUD* pHudItem = pWeaponItem->GetHUD();
		if (pHudItem) {
			CKinematicsAnimated* pHudVisual = smart_cast<CKinematicsAnimated*>(
				pHudItem->Visual()
			);
			if (pHudVisual) {
				MotionID motion = pHudVisual->ID_Cycle(M);
				if (motion) pHudVisual->PlayCycle(motion, bMixIn);
			}
		}
		else {
			ai().script_engine().script_log(
				ScriptStorage::eLuaMessageTypeError,
				"ScriptGameObject : attempt to call ActorPlayHudAnimation method for non-pHudItem object"
			);
		}
	}
	else {
		ai().script_engine().script_log(
			ScriptStorage::eLuaMessageTypeError,
			"ScriptGameObject : attempt to call ActorPlayHudAnimation method for non-pWeaponItem object"
		);
	}
}


CScriptGameObject* CScriptGameObject::GetHolderOwner() const {
	CHolderCustom* pHolderItem = smart_cast<CHolderCustom*>(&object());
	if (pHolderItem) return (pHolderItem->Owner())->lua_game_object();
	else {
		ai().script_engine().script_log(
			ScriptStorage::eLuaMessageTypeError,
			"ScriptGameObject : attempt to call GetHolderOwner method for non-pHolderItem object"
		);
	}
	return 0;
}

u32 CScriptGameObject::ActorGetState() const {
	CActor* pActor = smart_cast<CActor*>(&object());
	
	if (pActor) {
		return pActor->get_state();
	}
	else {
		ai().script_engine().script_log(
			ScriptStorage::eLuaMessageTypeError,
			"ScriptGameObject : attempt to call ActorPlayHudAnimation method for non-pActor object"
		);
	}
	return 0;
}

u32 CScriptGameObject::GetHudItemState() const {
	CHudItem* pHudItem = smart_cast<CHudItem*>(&object());
	
	if (pHudItem) {
		return pHudItem->GetState();
	}
	else {
		ai().script_engine().script_log(
			ScriptStorage::eLuaMessageTypeError,
			"ScriptGameObject : attempt to call GetHudItemState method for non-pHudItem object"
		);
	}
...
void CScriptGameObject::SetHudItemState(u32 value) {
	CHudItem* pHudItem = smart_cast<CHudItem*>(&object());
	
	if (pHudItem) {
		return pHudItem->SetState(value);
	}
	else {
		ai().script_engine().script_log(
			ScriptStorage::eLuaMessageTypeError,
