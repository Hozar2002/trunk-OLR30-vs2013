...
void CUICharacterInfo::Init(float x, float y, float width, float height, CUIXml* xml_doc)
{
	...
	// bio
	if (xml_doc->NavigateToNode("bio_static", 0))
	{
		pItem = m_icons[eUIBio] = xr_new<CUIStatic>();
		xml_init.InitStatic(*xml_doc, "bio_static", 0, pItem);
		pItem->SetElipsis(CUIStatic::eepEnd, 1);
		AttachChild(pItem);
		pItem->SetAutoDelete(true);
	}

	if (xml_doc->NavigateToNode("bio_caption", 0))
	{
		pItem = m_icons[eUIBioCaption] = xr_new<CUIStatic>();
		xml_init.InitStatic(*xml_doc, "bio_caption", 0, pItem);
		AttachChild(pItem);
		pItem->SetAutoDelete(true);
	}
...
void CUICharacterInfo::InitCharacter(u16 id)
{
	...
	if (m_icons[eUIBio])
	{
		m_icons[eUIBio]->SetText(chInfo.Bio().size() > 100 ? "см. профиль" : chInfo.Bio().c_str());
	}
