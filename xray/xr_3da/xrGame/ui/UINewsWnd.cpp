...
CUINewsTalkWnd::CUINewsTalkWnd()
{}

CUINewsTalkWnd::~CUINewsTalkWnd()
{}

void CUINewsTalkWnd::Init(LPCSTR xml_name, LPCSTR start_from)
{
	string512 pth;
...
void CUINewsTalkWnd::Init()
{
	Init(NEWS_XML, "");
}

void CUINewsTalkWnd::LoadNewsTalk()
{
	UIScrollWnd->Clear();
...
void CUINewsTalkWnd::AddNews()
{
	m_flags.set(eNeedAdd, TRUE);
}

void CUINewsTalkWnd::AddNewsTalkItem(GAME_NEWS_DATA& news_data)
{
	CUIWindow*				itm = NULL;
	switch (news_data.m_type){
	case GAME_NEWS_DATA::eNews:{
...
void CUINewsTalkWnd::Show(bool status)
{
	if (status)
		LoadNewsTalk();
	else
