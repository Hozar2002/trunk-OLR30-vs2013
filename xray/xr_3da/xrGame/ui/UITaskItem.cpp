...
static const u32 selected_task_color = color_rgba(0,0,0xff,0xff);

static CUIStatic* g_old_caption = NULL;
static u32 g_old_caption_color = 0;
...
void CUITaskItem::OnItemClicked(CUIWindow*, void*)
{
	...
	//if (GetCaptionStatic()) {
	//	if (g_old_caption) {
	//		g_old_caption->SetTextColor(g_old_caption_color);
	//	}

	//	g_old_caption = GetCaptionStatic();

	//	g_old_caption_color = GetCaptionStatic()->GetTextColor();
	//	GetCaptionStatic()->SetTextColor(selected_task_color);
	//}


	//}
	//catch (...) {
	//	Msg("!!!Error in g_old_caption");
	//}
...
void CUITaskRootItem::Init			()
{
	...
	m_descText = xr_new<CUIStatic>();
	m_descText->SetAutoDelete(true);
	AttachChild(m_descText);
	...
	xml_init.InitStatic			(uiXml, "task_root_item:desc_text",		0,  m_descText);
...
void CUITaskRootItem::SetGameTask(CGameTask* gt, u16 obj_idx)				
{
	...
	m_descText->SetText(*stbl.translate(m_GameTask->m_textDesc));
	m_descText->AdjustHeightToText();
	m_descText->SetWndPos(m_descText->GetWndPos().x, m_captionStatic->GetWndPos().y + m_captionStatic->GetHeight() + 3.0f);
	...
	m_captionTime->SetWndPos(m_captionTime->GetWndPos().x, m_captionStatic->GetWndPos().y + m_captionStatic->GetHeight() + m_descText->GetHeight() + 3.0f);

	//float h = _max	(m_taskImage->GetWndPos().y+m_taskImage->GetHeight(),m_captionTime->GetWndPos().y+m_captionTime->GetHeight()); // default with m_taskImage
	float h = _max	(0,m_captionTime->GetWndPos().y+m_captionTime->GetHeight());
...
CUITaskSubItem::CUITaskSubItem		(CUIEventsWnd* w, CUITaskRootItem* root_item)
:inherited(w)
{
...
void CUITaskSubItem::Init			()
{
	...
	//AddCallback						("m_showDescriptionBtn",BUTTON_CLICKED,CUIWndCallback::void_function(this, &CUITaskSubItem::OnItemClicked));
...
void CUIUserTaskEditWnd::Init					()
{
	...
	Register			(m_btnOk);
	AddCallbackAddCallback			(m_btnOk->WindowName(),BUTTON_CLICKED,boost::bind(&CUIUserTaskEditWnd::OnOk,this));
